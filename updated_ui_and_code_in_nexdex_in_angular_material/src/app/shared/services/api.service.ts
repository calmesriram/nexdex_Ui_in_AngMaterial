...
var web3 = new Web3(new Web3.providers.HttpProvider(environment.testnet_uri));
...
export class ApiService {  
  ...
  public WalletSocket_address = environment.WalletSocket_address;
  public zkAssetHandler_address = environment.zkAssetHandler_address;
  public ACE_address = environment.ACE_address;
  public explorer_url;
  ...
  constructor(
    ...
    this.explorer_url = environment.testnet_url;
  ...
  public async ecverify(pvt): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      this.http.post(this.baseurl + '/add_owner_address/', data).subscribe(res => {
        if (res['status'] == false && res['data'] == "User Session Expired  Need to login again") {
